...
pub struct Select {
    ...
    /// Flag that defines if the select works in optimistic mode. (Default: ``true` if no `state_topic` defined, else `false`.)`
...
impl Select {
    pub fn new(command_topic: String, options: Vec<String>) -> Select {
        Select {
            availability: None,
